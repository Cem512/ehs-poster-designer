...
export default function Toolbar() {
  ...
  const tools: { tool: ActiveTool; icon: typeof MousePointer2; label: string; shortcut?: string }[] = [
  ...
  return (
    ...
    >
      {/* Left panel toggle */}
      <ToolbarButton icon={PanelLeftOpen} label="Toggle Left Panel" onClick={toggleLeftPanel} />
      ...
      {/* Image upload */}
      <ToolbarButton icon={ImagePlus} label="Add Image" onClick={handleImageUpload} />
      <input
        ref={fileInputRef}
        type="file"
        accept="image/*"
      ...
      {/* Undo/Redo */}
      <ToolbarButton icon={Undo2} label="Undo (Ctrl+Z)" onClick={handleUndo} disabled={!canUndo} />
      <ToolbarButton icon={Redo2} label="Redo (Ctrl+Shift+Z)" onClick={handleRedo} disabled={!canRedo} />
      ...
      {!isMobile && (
        <>
          ...
          {/* View options */}
          <ToolbarButton icon={Grid3x3} label="Toggle Grid (G)" active={gridVisible} onClick={toggleGrid} />
          <ToolbarButton icon={Ruler} label="Toggle Snap" active={snapEnabled} onClick={toggleSnap} />
          <ToolbarButton icon={Magnet} label="Smart Guides" active={guidesVisible} onClick={toggleGuides} />
          <ToolbarButton icon={Eye} label="Readability Check" active={readabilityOverlayVisible} onClick={toggleReadabilityOverlay} />
          <ToolbarButton icon={Contrast} label="Contrast Check" active={contrastOverlayVisible} onClick={toggleContrastOverlay} />
        </>
      )}
      ...
      <div className="relative">
        ...
        >
          ...
          {!isMobile && (
            <span>
              {posterDoc.sizeKey} {posterDoc.orientation} — {posterDoc.viewingDistance}m
            </span>
          )}
        </button>
          ...
          <>
            ...
            >
              ...
              <div className="px-3 py-2 border-b" style={{ borderColor: 'var(--color-border)' }}>
                ...
                <p className="text-xs">
                  {posterDoc.sizeKey} ({posterDoc.size.width}×{posterDoc.size.height} mm) • {posterDoc.orientation}
                </p>
                <p className="text-xs" style={{ color: 'var(--color-text-muted)' }}>
                  {posterDoc.theme.label} • {posterDoc.viewingDistance}m viewing
                </p>
              </div>

              <MenuButton
                icon={Settings}
                label="Poster Settings…"
                subtitle="Theme, orientation, viewing distance"
                onClick={() => {
                  setMenuOpen(false);
              ...
              <MenuButton
                icon={FilePlus2}
                label="New Poster…"
                subtitle="Discard everything and start fresh"
                onClick={() => {
                  setMenuOpen(false);
      ...
      {/* Right panel toggle */}
      <ToolbarButton icon={PanelRightOpen} label="Toggle Right Panel" onClick={toggleRightPanel} />
        ...
        >
          ...
          >
            ...
            <div className="flex items-center justify-between px-5 py-3 border-b" style={{ borderColor: 'var(--color-border)' }}>
              <h3 className="text-base font-semibold">Poster Settings</h3>
              <button
            ...
            <div className="px-5 py-4 space-y-5 max-h-[70vh] overflow-y-auto">
              ...
              <p className="text-xs rounded-lg p-2.5" style={{ backgroundColor: 'var(--color-bg)', color: 'var(--color-text-muted)' }}>
                These changes apply live without affecting your canvas content. To change paper size, use "New Poster".
              </p>
                ...
                <div className="flex gap-2">
                  {(['portrait', 'landscape'] as const).map((o) => (
                    <button
                      ...
                      className="flex-1 py-2 px-3 rounded-lg text-sm text-center transition-all capitalize"
                      style={{
                ...
                <div className="space-y-1.5">
                  {SAFETY_THEMES.map((theme) => (
                    <button
                      ...
                      style={{
                        backgroundColor: posterDoc.theme.label === theme.label ? 'var(--color-surface-hover)' : 'transparent',
                        border: `1.5px solid ${posterDoc.theme.label === theme.label ? theme.primary : 'var(--color-border)'}`,
                      }}
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-[10px] truncate" style={{ color: 'var(--color-text-muted)' }}>
                          Signal word: {theme.signalWord}
                        </div>
                      </div>
                      {posterDoc.theme.label === theme.label && (
                        <span className="text-[10px] font-medium shrink-0" style={{ color: theme.primary }}>Active</span>
                      )}
                    </button>
                  ))}
                </div>
              ...
              <div>
                <label className="text-xs font-medium mb-2 block">
                  Viewing Distance: <strong>{posterDoc.viewingDistance}m</strong>
                </label>
                <input
                  type="range"
                  min={1}
                ...
                <div className="flex justify-between text-[10px] mt-1" style={{ color: 'var(--color-text-muted)' }}>
                  <span>1m (close)</span>
                  <span>10m (warehouse)</span>
                  <span>20m (outdoor)</span>
                </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <span style={{ color: 'var(--color-text-muted)' }}>
                        Min. readable text: <strong style={{ color: 'var(--color-text)' }}>{minPt.toFixed(0)} pt</strong>
                        {minPt > 72 && (
                          <span style={{ color: '#F59E0B' }}> — larger than standard title size</span>
                        )}
                        {minPt <= 72 && minPt > 36 && (
                          <span> — heading size or larger</span>
                        )}
                        {minPt <= 36 && (
                          <span style={{ color: '#22c55e' }}> — body text readable</span>
                        )}
                      </span>
                    </div>
                  );
      ...
      {confirmNewOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center" style={{ backgroundColor: 'rgba(0,0,0,0.6)' }}>
          <div
            ...
            style={{ backgroundColor: 'var(--color-surface)', border: '1px solid var(--color-border)' }}
          >
            <h3 className="text-base font-semibold mb-2">Start Over?</h3>
            <p className="text-sm mb-4" style={{ color: 'var(--color-text-muted)' }}>
              This will discard your current poster and return to the setup wizard. Any unsaved changes will be lost.
            </p>
